...
## 📖 文件慣用符號（先看這個、後面 stage 都會用到）

本專案大量使用以下符號，第一次讀不熟的話先參考：

| 符號 | 意思 | 範例 |
|---|---|---|
| **§** | 章節（section sign，學術 / 法律慣例）| `§Harness Engineering` 表示「Harness Engineering 這一節」 |
| **🔄** | 共用 hub —— Track A + Track B 兩條路徑都會用到的 stage | Stage 5 / Stage 8 在 README 標 🔄 |
| **⭐ / ⭐⭐ / ⭐⭐⭐⭐⭐** | **教材重要性 / 難度**（星數越多越關鍵 / 越進階）| Stage 3 ⭐ = 必修入門、Stage 5 ⭐⭐ = 兩條路徑都要學 |
| **★ 8k+** | **GitHub 星數**（社群熱度，**跟教材重要性無關**）| `★ 8k+` 表示那個 repo 有 8000+ stars |
| ⭐ vs ★ | 兩者不同：**⭐ = 教材重要性、★ = 社群熱度**。同一個列表可能兩者並用 | — |
