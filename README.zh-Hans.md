...
> 📖 **本页惯用符号**：`🔄` = 两条路径共用 hub（Stage 5 / Stage 8）；`⭐` = 教材重要性 / 难度（不是 GitHub 星数）；`★ 8k+` 才是 GitHub 星数；`§` = 章节（如 §Harness Engineering）。完整符号表见 [`resources/glossary.zh-Hans.md`](resources/glossary.zh-Hans.md)。
