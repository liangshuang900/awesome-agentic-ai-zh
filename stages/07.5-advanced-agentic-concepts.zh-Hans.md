...
1. **incident**（2026-05-13、Phase D 操作者跳过 preset、shipped 一个 mirror drift）
2. **observed-failure-modes.md** 命名为 F14（META-FAILURE 类别）
3. **codify**：写进 `multi-locale-mirror-sync.yml` + `cross_document_link_text_parity` check（v0.2.3）
4. **retrospective test**：对 commit `4a60742` 跑、抓到 9+ real drift（6 个原 audit 漏的、3 个 audit 已知的）
5. **mandatory rule**：CLAUDE.md Complex Task Protocol 写进「trigger 触发时 preset 必跑」规则
