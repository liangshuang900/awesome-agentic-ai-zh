...
1. **incident**（2026-05-13、Phase D 操作者跳過 preset、shipped 一個 mirror drift）
2. **observed-failure-modes.md** 命名為 F14（META-FAILURE 類別）
3. **codify**：寫進 `multi-locale-mirror-sync.yml` + `cross_document_link_text_parity` check（v0.2.3）
4. **retrospective test**：對 commit `4a60742` 跑、抓到 9+ real drift（6 個原 audit 漏的、3 個 audit 已知的）
5. **mandatory rule**：CLAUDE.md Complex Task Protocol 寫進「trigger 觸發時 preset 必跑」規則
